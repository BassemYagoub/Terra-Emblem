...
public class DialogueManager : MonoBehaviour {
    ...
    //REALLY BAD WAY OF STORING DIALOGUES (but I'm being lazy) => to change if more dialogues are to be put in game
    void ChargeDialogues() {
        ...
        if (sceneName == "Level0") {
            ...
            dialogues.Enqueue(new Dialogue("Zack", "..."));
            dialogues.Enqueue(new Dialogue("Laydee Ron", "..."));
            dialogues.Enqueue(new Dialogue("Lee", "(Ok...)"));
        }
    }
    ...
    public void GoToNextDialogue() {
        if(dialogues.Count > 0) {
            if (textAnimationDone) {
                ...
                if(dialogueInterlocutor.text == "Laydee Ron") { //don't want complete name in ohter game UI
                    CameraMovement.FollowUnit("Laydee");
                }
                else {
                    Debug.Log(dialogueInterlocutor.text);
        ...
        if (!textAnimationDone) { //clicking when text is animating => pass animation
            textAnimationDone = true;
            dialogueText.text = currentDialogue.lines;
        }
    }
